...
def plot_d_luminosity(ax, z, d_L, d_L_bounds, label, color, fill_alpha, h):
    ...
    if d_L_bounds is not None:
        ...
        ax.plot(z, h*d_L_bounds[0], color=color, lw=0.5, alpha = 0.6)
        ax.plot(z, h*d_L_bounds[1], color=color, lw=0.5, alpha = 0.6)
...
def setup_luminosity_plots():
    ...
    for H_0, color in configurations:
        temp_list = []
    ...
    for (H_0, color,), d_L_values in zip(configurations, d_L_for_fill):
